...
module.exports = async (client, message) => {
            ...
            try {
              if (levelData) {
                await client.channels.cache
                  .get(levelData.Channel)
                  .send({
                    content: `Woah! <a:tadaG4:1209123499009122334> ${message.author}, you just leveled up to level **${user.level}**!`,
                  })
                  .catch(() => { });
              } else {
  ...
  try {
    Schema.findOne(
      ...
      async (err, data) => {
        ...
        await lastStickyMessage.delete({ timeout: 500 });
  ...
  if (
    ...
    client.embed({
      title: "Hey " + message.author.displayName + " 👋🏻, I'm **NexusX**",
      desc: "Start using me today! You can use me with either **`/`** or **`X`**.",
      fields: [
        {
          name: "❓┆I don't see any slash commands",
          value: "If you can't see slash commands, it might be because the bot doesn't have the necessary permissions.",
        },
        {
          name: "❓┆Need support?",
          value: "If you have any questions or need support, feel free to join our [support server](" + client.config.discord.serverInvite + ")!",
        },
        {
          name: "🐞┆Found a bug?",
          value: "If you encounter any bugs, please report them using the `/report bug` command!",
        },
        {
          name: "➕┆Want to add me to your server?",
          value: "Join our support server to ask more and find the invite link to add me to your server!",
        },
      ],
      components: [row],
        },
