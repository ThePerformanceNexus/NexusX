...
module.exports = async (client, interaction, args) => {
    ...
    var result = Math.ceil(Math.random() * 6); // Reduced the random number range to 1-6

    Schema.findOne({ Guild: interaction.guild.id, User: user.id }, async (err, data) => {
        if (data) {
            let money = parseInt(interaction.options.getNumber('amount'));
            if (!money) return client.errUsage({ usage: "crash [amount]", type: 'editreply' }, interaction);
            if (money > 5000) return client.errNormal({ error: `The max bet amount is 5000 ${client.emotes.economy.coins}`, type: 'editreply' }, interaction);
            ...
            client.embed({
                ...
                setInterval(() => {
                    if (index === result + 1) { return }
                        ...
                        index += 1;
                        multiplier += 0.10; // Reduced the multiplier increment to 0.10
