  ...
  if (perms == false) return;
    ...
    const row = new Discord.ActionRowBuilder() 
        .addComponents(
            ...
            new Discord.ButtonBuilder()
                ...
                .setEmoji('<:cross_:1214201181551853659>')
                .setStyle(Discord.ButtonStyle.Danger),
        );

    client.embed({
        title: `⏰・Reset Casino`,
        desc: `Are you sure you want to reset the Casino? this action can not be undone!`,
        components: [row],
        type: 'editreply',
    }, interaction)
    ...
    interaction.channel.awaitMessageComponent({ filter, componentType: Discord.ComponentType.Button, time: 60000 }).then(async i => {
        if (i.customId == "eco_go") {
            ...
            client.succNormal({
                text: `The casino has been successfully reset in this guild!`,
                components: [],
                type: 'editreply'
            }, interaction);
        }
        ...
        if (i.customId == "eco_stop") {
            client.errNormal({
                error: `The reset has been cancelled!`,
                components: [],
                type: 'editreply'
            }, interaction);
        }
        ...
        .catch(() => {
            client.errNormal({
                error: "Time's up! Cancelled the reset!",
                type: 'editreply'
            }, interaction);
        });
}
